...
- [Mihon](https://github.com/briones-agent/mihon): Isolated approach using AAR (SDK 57 canary) — Kotlin/Compose manga reader (formerly Tachiyomi) built with the fused mode from [PR #47921](https://github.com/expo/expo/pull/47921); based on the `v0.20.1` release tag (upstream `main` currently crashes debug builds on its own, [mihonapp/mihon#3594](https://github.com/mihonapp/mihon/pull/3594)), launched from an “Expo demo” row in the More tab; needed an explicit `androidx.localbroadcastmanager` dep because the fused AAR's Material 1.12→1.13 bump drops the legacy androidx transitive chain the host relied on ([recording](https://raw.githubusercontent.com/expo/expo-brownfield-examples/media/04-mihon/recording.mp4))
